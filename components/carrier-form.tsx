...
const inputClass =
  "w-full px-3 py-2.5 rounded-md text-sm border outline-none focus:ring-2 bg-white transition-shadow";
const inputStyle = { borderColor: "#D2DCE6" };
const labelClass = "mono block text-xs tracking-widest mb-1.5";
const sectionClass = "mono text-xs tracking-[0.25em] mb-4";

const EQUIPMENT = ["Dry Van", "Reefer", "Flatbed", "Box Truck", "Power Only", "Other"];
...
export default function CarrierForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const data = { ...Object.fromEntries(new FormData(form).entries()), equipment };
    ...
    return (
      <div className="rounded-lg p-10 text-center" style={{ background: brand.paper }}>
        ...
        <p className="text-sm" style={{ color: brand.slate }}>
          Our carrier team will review your info and reach out to complete setup.
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      {/* Section: company */}
      <p className={sectionClass} style={{ color: brand.accent }}>
        YOUR COMPANY
      </p>
      ...
      <div className="mb-4 grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <input id="company" name="company" required className={inputClass} style={inputStyle} />
        </div>
        <div>
          <label htmlFor="contact" className={labelClass} style={{ color: brand.slate }}>
            CONTACT NAME
          </label>
          <input id="contact" name="contact" required className={inputClass} style={inputStyle} />
        </div>
      </div>
      ...
      <div className="mb-8">
        <label htmlFor="lanes" className={labelClass} style={{ color: brand.slate }}>
